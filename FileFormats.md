...
Both CLA and CUK Origins uses \*.XML for some of the interfaces, while Shattered Memories uses \*.XAML and some files uses \*.XML, but what makes it deserve a whole section is their obfuscation. A further investigation will require to read game functionality, but that is far from my (IWILLCRAFT) knownledge.
...
Similar to Shattered Memories having the filenames hashed in the \*.ARC all Climax's SH games has string tables uses hashed IDs from strings, the hashing variates from in each game (CLA and CUK Origins, and SHSM). For now the only one having been able to fully recreated has been the one used on SHSM.

```
Silent Hill: Shattered Memories string table hashing method
