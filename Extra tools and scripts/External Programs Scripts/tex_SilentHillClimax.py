        ...
        if self.deviceId == 3:
            ...
            if depth == 4:
                ...
                if width * height > 512:
                    ...
                    pixelBuff = unswizzle4l(pixelBuff, width, height)
            ...
            texFormatExt = rasterFormat & 0xf000
            texFormat = rasterFormat & 0x0f00
            
            texData = bytearray()
            
            if texFormatExt == 0x2000:
                palette = self.bs.readBytes(1024)
            elif texFormat == 0x0300 or texFormat == 0x0200 or texFormat == 0x0100:
                pass
            else:
            ...
            if texFormat == 0x0200:
                texData = rapi.imageDecodeRaw(pixelBuff, width, height, "r5g6b5")
            elif texFormat == 0x0300:
                texData = rapi.imageDecodeRaw(pixelBuff, width, height, "r4g4b4a4")
            elif texFormat == 0x0200 or texFormat == 0x0100:
                texData = rapi.imageDecodeRaw(pixelBuff, width, height, "r5g5b5a1")
            else:
                texData = rapi.imageDecodeRawPal(pixelBuff, palette, width, height, depth, "r8g8b8a8")
...
def unswizzle4(buffer, width, height):
    ...
    for i in range(width * height//2):
        pixels[i*2] = buffer[i] & 0xf
        pixels[i*2+1] = (buffer[i] >> 4) & 0xf
    pixels = rapi.imageUntwiddlePS2(pixels, width, height, 8)
    for i in range(width*height//2):
        pixels[i] = ((pixels[i*2+1] << 4) | pixels[i*2])
    return pixels


def unswizzle4l(buffer, width, height):
    pixels = bytearray(width * height)
    for i in range(width * height//2):
        index = buffer[i]
        id2 = (index >> 4) & 0xf
